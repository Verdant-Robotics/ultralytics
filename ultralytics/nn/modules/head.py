...
class DetectTunableHead(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        self.n_sources = self.nc // 2  if self.nc != 1 else self.nc # number of sources (nc // 2)
...
class DetectAndSeg(Detect):
    def __init__(self, nc=80, ch=(), seg_ch_num=1):
        ...
        c4 = max(16, ch[0] // 4)
        self.cv_seg_obj = nn.ModuleList(nn.Sequential(Conv(x, c4, 3), nn.Conv2d(c4, 2, 1)) for x in ch) # 1(sh obj) + 1(unsh obj) + seg clses
        self.cv_seg_cls = nn.ModuleList(nn.Sequential(Conv(x, c4, 3), nn.Conv2d(c4, seg_ch_num, 1)) for x in ch) # 1(sh obj) + 1(unsh obj) + seg clses

    def forward(self, x):
        """Concatenates and returns predicted bounding boxes and class probabilities."""
        shape = x[0].shape  # BCHW
        for i in range(self.nl):  # per detection scale
            x[i] = torch.cat((self.cv2[i](x[i]), self.cv3[i](x[i]), self.cv_seg_obj[i](x[i]), self.cv_seg_cls[i](x[i])), 1)
